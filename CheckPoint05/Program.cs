...
namespace CheckPoint05
{
    ...
    class Program
    {
        ...
        {
            List<CStudent> listData = new List<CStudent>(); // 데이터 저장 리스트
            bool IsLoop = true;
            ...
            do
            {
                ...
                Console.Write("(1) id 정렬 (2) 성적순 정렬 (3) 국어점수 정렬 (4) 특정 점수 이상 검색 (5) 특정 점수 이하 검색 (0) 나가기");
        ...
        // 2,
        // 3,
        static void InitData(List<CStudent> _listData)  // init은 초기화
        {
        ...
        static void FindDataUp(List<CStudent> _listData, bool isUP)
        {
            ...
            finally
            {
                ...
                {
                    Console.Clear();
                    Console.WriteLine("입력값 {0} 잘못된 입력입니다. 너무 작은 숫자입니다.", num);
                }
                if (num > 300)
                {
                    Console.Clear();
                    Console.WriteLine("입력값 {0} 잘못된 입력입니다. 너무 큰 숫자입니다.", num);
                }
            }
            if (num >= 0 && num <= 300)
            {
