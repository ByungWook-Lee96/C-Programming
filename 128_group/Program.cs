...
namespace _128_group
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                // group by에서 true값이 들어오면 합이 150보다 작은게 나옴
            ...
            foreach(var data in QueryData)
            {
                if (false == data.Key) //합이 150보다 큰 경우
            ...
            {
                Console.WriteLine("합이 150보다 큰 경우: " + listMaxStudents[i]._name);
            }
            for (int i = 0; i < listMinStudents.Count; i++)
            {
                Console.WriteLine("합이 150보다 작은 경우: " + listMinStudents[i]._name);
            }
        }
    }
}
